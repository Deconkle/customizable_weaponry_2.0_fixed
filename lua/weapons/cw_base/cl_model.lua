...
function SWEP:GetTracerOrigin()
	...
	if IsValid(self:getMuzzlePosition()) then
...
function SWEP:offsetBoltBone()
	...
	if !self.BoltBoneID then return false end
	if type(self.BoltBoneID) == "number" or type(self.BoltBoneID) == "Number" then return false end
	if !IsValid(self.BoltBoneID) then return false end
...
function SWEP:resetM203Anim()
	...
	if !(type(vm) == "Entity") then return end
...
function SWEP:getBoneOrientation(boneId)
	if !(type(boneId) == "Number" or type(boneId) == "number") then return end
...
function SWEP:_drawAttachmentModel(data)
	...
	if (ang) or !(ang == nil) or !(type(ang) == nil) then
	...
	if self.DrawTraditionalWorldModel then
		...
		if IsValid(wm) then
			if IsValid(self.Owner) then
				...
				pos, ang = GetBonePosition(self.Owner, bone and bone or 0)
